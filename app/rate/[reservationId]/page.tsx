...
const STEP_ORDER: Step[] = ['rating-type', 'stars', 'tags', 'comment', 'success']
...
function ProgressBar({ step }: { step: Step }) {
  const progressSteps = STEP_ORDER.filter((s) => s !== 'success')
...
export default function RatingPage({ params }: PageProps) {
  ...
  const { reservationId: rawId } = React.use(params)
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 md:p-8">
              ...
              [
                {
                  ...
                  description: 'How was your arrival and check-in process?',
                },
                {
                  type:        'stay' as const,
                  title:       'Overall Stay',
                  description: 'How was your overall experience during the stay?',
                },
              ] as const
        ...
        {step === 'stars' && (
          <div className={`space-y-6 ${fadeIn}`}>
            ...
            <StarSelector value={stars} onChange={(n) => { setStars(n); setStep('tags') }} />
              ...
              {FEEDBACK_TAGS.map((tag) => {
                ...
                return (
                  <button
                    ...
                    className={`py-2.5 px-3 rounded-xl border-2 text-sm font-medium transition-all duration-150 active:scale-[0.97]
                      focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500
                      ${
                        active
                          ? tag.positive
